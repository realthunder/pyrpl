...
module tb_fft_native_stall;
    ...
    localparam int OUT_STALL_CYC   = 400;   // ...for this many cycles
    localparam int IN_GAP_FRAME    = 4;     // deassert s_tvalid during this input frame
    localparam int IN_GAP_BEAT     = 70;    // ...starting at this input beat
    localparam int IN_GAP_CYC      = 350;   // ...for this many cycles
    ...
    logic [DSZ-1:0] fmax; integer fmaxbin;
        ...
        if (m_tvalid && m_tready) begin
            ...
            if (m_tlast) begin
                $display("  [t=%0t] OUTPUT frame %0d: peak bin=%0d (expect %0d) mag=%0d  %s",
                         $time, oframe, fmaxbin, K, fmax, (fmaxbin==K)?"OK":"*** SHIFTED ***");
                oframe <= oframe + 1;
