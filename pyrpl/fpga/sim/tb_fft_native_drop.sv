...
module tb_fft_native_drop;
    ...
    localparam int BADF            = 3;   // frame to shorten
    localparam int DROP            = 16;  // beats dropped from BADF (64 samples)    // ...starting at this input beat
    ...
    initial begin
        ...
        for (f=0; f<NFRAMES; f++) begin
            int nbeats; nbeats = (f==BADF) ? BEATS-DROP : BEATS;
            if (f==BADF) $display("  [t=%0t] >>> frame %0d: DROPPING %0d beats (feeding %0d not %0d)",$time,f,DROP,nbeats,BEATS);
