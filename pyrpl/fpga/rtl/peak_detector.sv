...
localparam PIPLINE = 4-1;
localparam S_IDLE = 0;
localparam S_STREAM = 1;
localparam S_DETECT = 2;
localparam S_DETECT1 = S_DETECT+PIPLINE+1;
localparam S_DETECT2 = S_DETECT1+PIPLINE+1;
localparam S_DETECT3 = S_DETECT2+PIPLINE+1;
...
logic [SSZ+DSZ*2-1:0] sum_sq[PIPLINE: 0];      // Max value: 2^SSZ * 2^DSZ * 2^DSZ
...
// peak * N - sum
logic [SSZ+DSZ-1:0] scaled_diff [PIPLINE: 0];
// peak2 * N - sum
logic [SSZ+DSZ-1:0] scaled_diff2 [PIPLINE: 0]; 
...
logic [(SSZ+DSZ)*2-1:0] threshold [PIPLINE: 0];
...
if (resetn == 0) begin
    ...
    if (current_state == S_IDLE && maxi_valid) begin
        ...
        S_sq[0] <= sum * sum; // S^2
        N_S2[0] <= count * sum_sq[PIPLINE]; // N * S2
        ...
        scaled_diff_sq[0] <= scaled_diff[PIPLINE] * scaled_diff[PIPLINE];
        scaled_diff2_sq[0] <= scaled_diff2[PIPLINE] * scaled_diff2[PIPLINE];
        ...
        if (scaled_diff_sq[PIPLINE] < threshold[PIPLINE]) begin
            peak_idx <= 0;
            peak2_idx <= 0;
        end else if (scaled_diff2_sq[PIPLINE] < threshold[PIPLINE])
            peak2_idx <= 0;
        sum_o <= sum;
        count_o <= count;
        current_state <= S_IDLE;
