...
logic [ FSZ-1: 0]   fft_hist_rdata_up_a;
logic [ FSZ-1: 0]   fft_hist_rdata_down_a;
...
fft_proc #(.ASZ(ASZ), .QSZ(QSZ), .DSZ(DSZ), .FSZ(FSZ), .RSZ(RSZ), .HSZ(HSZ)) fft_a (
   ...
   .fft_scan_frame_cnt (fft_scan_frame_cnt),
...
if (adc_rstn_i == 1'b0) begin
   ...
   casez (sys_addr[19:0])
     ...
     20'h00030 : begin sys_ack <= sys_en;          sys_rdata <= fft_nfft<RSZ-1?fft_frame_cnt:(fft_frame_cnt<<1); end
     ...
     */
     ...
     20'h00194 : begin sys_ack <= sys_en;          sys_rdata <= fft_dbg_cnt                         ; end
