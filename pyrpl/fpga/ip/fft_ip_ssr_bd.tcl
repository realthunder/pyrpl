...
# ---- Clock / reset --------------------------------------------------------
foreach pin [list pre_0/ap_clk post_0/ap_clk] {
    connect_bd_net [get_bd_ports aclk] [get_bd_pins $pin]
}
foreach pin [list pre_0/ap_rst_n post_0/ap_rst_n] {
    connect_bd_net [get_bd_ports aresetn] [get_bd_pins $pin]
}
for {set ch 0} {$ch < $fft_ssr} {incr ch} {
    connect_bd_net [get_bd_ports aclk]    [get_bd_pins xfft_$ch/aclk]
    connect_bd_net [get_bd_ports aresetn] [get_bd_pins xfft_$ch/aresetn]
}
...
for {set ch 0} {$ch < $fft_ssr} {incr ch} {
    connect_bd_intf_net [get_bd_intf_pins pre_0/m_axis_data${ch}] \
                        [get_bd_intf_pins xfft_${ch}/S_AXIS_DATA]
    connect_bd_intf_net [get_bd_intf_pins pre_0/m_axis_cfg${ch}] \
                        [get_bd_intf_pins xfft_${ch}/S_AXIS_CONFIG]
    connect_bd_intf_net [get_bd_intf_pins xfft_${ch}/M_AXIS_DATA] \
                        [get_bd_intf_pins post_0/s_axis_data${ch}]
}
